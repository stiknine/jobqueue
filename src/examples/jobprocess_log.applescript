...
property process_log : true
property process_log_file_name : "process_log.txt"
		...
		if my checkName(this_name, this_folder) is true then
			try
				...
				if the number of items in these_items is 1 then
					...
					-- log processing details to file
					if process_log is true then
						-- get the image name using the basename command line command
						set imgName to do shell script "basename " & the quoted form of the POSIX path of these_items
						
						-- log image name and size W x H
						-- log message
						set process_log_msg to "Image: " & imgName & " Size: " & W & "W" & " x " & H & "H" & return
						-- log file and path
						set process_log_file to ((folder "log" of folder this_folder as text) & process_log_file_name)
						-- using the log file routine available in the jobqueue system
						my write_to_file(process_log_msg, process_log_file, true)
					end if
