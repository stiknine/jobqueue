...
-- this folder will be created on the desktop
property output_folder_name : "output"
-- text to append to new file names
property new_name : "_new.jpg"
...
property job_wait : false
-- delay for next job to be processed, in seconds
property job_wait_delay : 5
...
property job_groups : false
-- number per group
property job_group_count : 10
-- group folder name
property job_group_name : "group"
		...
		if my checkName(this_name, this_folder) is true then
			try
				...
				if the number of items in these_items is 1 then
					...
					-- get info of these items
					set item_info to info for these_items
					
					-- this will remove the filename extension 
					set item_name to do shell script "echo " & quoted form of the (name of the item_info) & "| sed 's|\\(.*\\)\\..*$|\\1|'"
					
					-- folder on the desktop to copy renamed files to
					set output_folder to the POSIX path of ((path to desktop folder) & output_folder_name as string)
					
					-- use the command line cp command to copy and rename the files, single quotes surround the filename
					do shell script "cp '" & the POSIX path of these_items & "' '" & output_folder & "/" & item_name & new_name & "'"
