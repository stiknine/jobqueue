...
property extensionList : {"html", "jpg"}
-- create a folder on the desktop named "jobtest" and 2 subfolders named "html" and "images"
property htmlFolder : (path to desktop folder) & "jobtest:html:" as string
property imageFolder : (path to desktop folder) & "jobtest:images:" as string
			...
			try
				...
				if the number of items in these_items is 1 then
					...
					repeat with x in these_items
						...
						-- get contents of this item
						set theFiles to entire contents of folder x
						
						-- copy html files
						repeat with a in theFiles
							with timeout of weeks seconds
								if class of a is not folder and (the name extension of a is in the extensionList) then
									if (the name extension of a is "html") then
										duplicate a as alias to folder htmlFolder with replacing
									end if
								end if
							end timeout
						end repeat
						...
						repeat with a in theFiles
							with timeout of weeks seconds
								if class of a is not folder and (the name extension of a is in the extensionList) then
									if (the name extension of a is "jpg") then
										duplicate a as alias to folder imageFolder with replacing
									end if
								end if
							end timeout
						end repeat
